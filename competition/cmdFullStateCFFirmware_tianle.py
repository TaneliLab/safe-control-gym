...
if __name__ == "__main__":
    ...
    while not timeHelper.isShutdown():
        ...
        if command_type == Command.FULLSTATE:
            ...
            # ---------------------add tianle interStepLearnc
            action = 0 # not using in interStepLearn
            # vicon_obs instead of obs
            # !! check args, is the same to simulation
            # interStepLearn takes pos(args[0]) and acc command(args[2])
            ctrl.interStepLearn(args, action, vicon_obs, reward, done, info)
